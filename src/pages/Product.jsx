...
const Product = ({ darkMode, setDarkMode }) => {
  ...
  useEffect(() => {
    // La pantalla sube cuando se carga este componente.
    const getProduct = async () => {
      window.scrollTo({ top: 0, behavior: 'smooth' });
      const res = await productById(id, setProduct, setColor ); // obtener la información específica del producto
      return res;
    };
    getProduct();
  }, [id]);
  ...
  const handleClick = () => {
    addToReduxCart(dispatch, setQuantity, product, quantity, color);
  };
  return (
