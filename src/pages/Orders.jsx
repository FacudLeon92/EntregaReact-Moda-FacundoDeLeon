...
const Orders = ({ userId }) => {
  ...
  return (
    <Container>
          ...
          {currentOrders.map((order) => {
            return (
              <Wrapper key={order._id}>
                ...
                <Cell
                  aria-label={`Pedido: ${order._id}`}
                  title={`Pedido: ${order._id}`}
                >
                  <Label>
                    Pedido: <ShoppingBasket />
                  </Label>
                  <Info>{order._id} </Info>
                </Cell>
                <Cell
                ...
                <Cell
                  aria-label={`Creada : ${formatCreatedAt(order.createdAt)}`}
                  title={`Creada: ${formatCreatedAt(order.createdAt)}`}
                >
                  <Label>Creada :</Label>
                  <Info>
      ...
      {orders.length === 0 ? (
        <SadFaceMsg text={'No se encontraron pedidos'} />
      ) : (
        <Pagination
